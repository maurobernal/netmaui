    ...
    private OidcClient CreateOidcClient()
    {
        var options = new OidcClientOptions
        {
            ...
            Policy = new Policy()
            {
                ...
                Discovery = new IdentityModel.Client.DiscoveryPolicy() { RequireHttps = true }
            }
        };

        options.BackchannelHandler = new HttpClientHandler() { ServerCertificateCustomValidationCallback = ValidateSSL };

        var oidcClient = new OidcClient(options);
        return oidcClient;
    }

    public bool ValidateSSL(HttpRequestMessage message, X509Certificate2? certificate, X509Chain? chain, SslPolicyErrors sslpolicyerrors)
    {
        return true;
    }
